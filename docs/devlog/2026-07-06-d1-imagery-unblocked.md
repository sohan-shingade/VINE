...
D1's imagery path had been blocked for two weeks — NextCloud 503'd since
...
1. **NextCloud came back** after the July 2 Ceph upgrade (verified by
   downloading a real 10.9 MB capture, not by trusting a 200 on the front page).
2. A systematic walk of the share found **`GIS/IHV-2026-05-26.kmz` with all 39
   ...
   locations — the sensor→block mapping D1 needed.
3. The same `GIS/` folder has **stitched Pix4DFields orthomosaics** — including
   ...
   vineyard on 2026-06-01. No stitching needed for those dates.
...
- **`d1_pipeline/webdav.py`** — a ~120-line NextCloud public-share client
  ...
  Zero new dependencies — `requests` was already core.
- **`d1_pipeline/imagery.py`** — flight discovery + capture grouping +
  size-checked band downloads. The interesting bug-shaped fact: **STAC's
  asset hrefs are stale.** The share tree was re-sorted after STAC indexing —
  whole flights moved between block folders — so hrefs 404 and STAC's block
  attribution can't be trusted. Flight-folder names and filenames are the
  stable keys; the reader indexes the tree (~60 cheap PROPFINDs) and treats
...
- **`d1_pipeline/geo.py`** — block polygons from the KMZ (stdlib KML parsing:
  ...
  10–18 GB; we read per-polygon windows, never whole files), and
...
- One flight (`H5/2026-01-08`): grouped into **611 captures, 582 with all four
  bands** — 582×5 files + 4 PPK sidecars = 2,914, matching the folder exactly.
- Downloaded one capture's G/R/RE/NIR TIFs (each exactly 10,087,424 bytes —
  the camera's fixed frame size doubles as an integrity check), read them with
  rasterio, computed **NDVI mean 0.272** — plausible for a dormant-season vineyard.
...
the share (~24 tool calls) and wrote `docs/data/imagery-share-layout.md`; the
implementation then consumed the brief, not the tree. Second, **codemap
shards** (`make codemap`): one generated markdown file per package with every
signature and import edge, so navigating the repo doesn't require opening
source files.
...
threshold baselines first, per ADR-0003 — nothing ships without beating the
rung below.
