...
1. **Real soil moisture is close to a random walk.** The naive baseline
   ...
   ~18 to 45 scale). Ridge regression on lags, rolling stats, and past weather
   is *worse than doing nothing* at every horizon (skill −0.8 to −1.0).
2. **Two of our three "wins" today were evaluation artifacts.** The unit tests
   missed them; an adversarial review pass caught them.
3. After fixing everything, **ARIMA is the first model to beat persistence**,
   by 2 to 3 %, positive in every fold at 48 h. Real, but not enough to ship.

Per ADR-0003, nothing ships without beating the baseline on honest held-out
data. **Persistence remains the champion.** That reads as a disappointing
result, but catching models that don't beat the baseline is exactly what the
evaluation harness was built to do.
...
Bonus lesson: don't put inline comments after values in `.env`, because dotenv
reads `KEY=   # comment` as the literal value `# comment`. Our `.env.example`
did exactly that and broke a test the moment it was copied verbatim.
...
- **Lead-time features** (`add_lead_time_features`): `precip_next_24h`,
  ...
  so they give the upper bound on what a real forecast could contribute). The
  harness drops any `_next_{k}h` column whose k doesn't match the horizon being
  scored, because a mismatched window reaches past the target time, which is a
  leak.
...
(each ~190 to 200 k rows over the same 5.5 months):
...
A model that gains 2 % on two sensors and loses 10 to 18 % on a third is not
reliable enough to serve; the sign of its skill depends on which probe you
point it at. **Ship decision, per ADR-0003: persistence is the D2 forecaster.**

The consolation prize is real, though. Unlike LS-1, the other sensors'
holdouts actually cross the 25.0 irrigation threshold, and there the
persistence alert ("will moisture be below threshold in h hours?") scores
precision and recall of **0.95 to 0.99**. Taking the latest reading as the
forecast and alerting on it works well as an irrigation advisor, and that's
what D6 will serve.
...
orchestrator *and* all three workers believed the first table. The only
reason today's devlog isn't announcing a fake +8 % is that one agent's entire
job was to disbelieve it. A pass/fail gate at the end would not have caught
it; evaluation-driven development needs a reviewer whose job is to attack the
results.
