...
exploratory when labels are unavailable. This is that slice, built with no
labels at all. `src/vine/d4_harvest/phenology.py` computes cumulative growing
degree-days (base 10 °C, Winkler window Apr 1 to Oct 31) at the vineyard
coordinates from public ERA5 daily weather, plus the dates when configured
literature phenology bands are crossed. Pure functions, 15 unit tests, YAML
config, a runner script, and a
[report](../reports/2026-08-04-d4-gdd-exploration.md) plus
[model card](../models/harvest/gdd-exploration.md) that both say plainly that
this is not a model, that it does not ship, and that it must not be used to
schedule a pick.
...
oracle-assisted upper bound rather than deployable evidence. This session runs
the validation that caveat demands: the same corrected, purged evaluation, but
fed from archived forecast data instead of realized future weather
...
The result splits by horizon. The 48 h aggregate edge survives real forecasts:
+5.3…+13.5% across all five probes, slightly wider than the oracle's
+3.5…+11.2%. The physical signal is real, in that rain arrives, ET removes
water, and two-day-ahead forecasts carry enough of it. But 24 h flips negative
on every probe (−2.8…−8.1%, worst folds to −2.448, i.e. nearly 3.5× persistence
error in the worst fold). At that horizon the model trusts day-1 forecast
precipitation that the realized weather did not deliver on the hours forecast,
an error the oracle backtest could not exhibit by construction.
...
failure is concentrated in fold-level forecast busts rather than a broken
mechanism, so the next lever is forecast-uncertainty-aware gating rather than a
new model family. Full numbers:
[D2 vintage validation](../reports/2026-08-04-d2-vintage-validation.md).
...
mode both times was the long-lived remote read path, with GDAL range-reading
two 4 GB `/vsicurl` rasters over a public NextCloud share for hours, which
...
1.000. Under the old bounding-box denominator, 9 of 39 blocks looked like they
had missing pixels, but the missing "pixels" were just the parts of each
bounding box outside the polygon. The whole-vineyard mosaic of 2026-06-01
in fact covers every block interior completely. The per-block distributions
are unchanged, with identical pixel counts and quantiles, because only the
quality denominator was ever wrong.
...
rank-disagreement flag and deserve extra care in review. The output is a review
queue rather than a diagnosis. It has no labels and no learned parameters, and
it cannot tell stress from phenology, shadow, or a processing artifact.
...
The computed véraison band crossings land around Sept 1 to 17, which is
implausibly late for Sonoma County. The cause is a window mismatch: the cited
literature bands accumulate from Jan 1 while this run's Winkler window starts
Apr 1, discarding January to March heat. That offset is a direct quantitative
demonstration that transplanted literature thresholds cannot substitute for
local calibration, which needs the mentor's harvest records. The
`sparkling_harvest_watch_UNCALIBRATED` band is a declared placeholder with no
source; it exists only as a slot to calibrate later.
