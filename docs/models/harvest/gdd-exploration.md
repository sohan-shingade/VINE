...
- **Architecture:** None. Deterministic arithmetic: daily GDD = max(mean(Tmax, Tmin) − 10 °C, 0), summed over the Winkler window Apr 1 – Oct 31 (Amerine & Winkler 1944), plus dates when configured literature phenology bands are crossed.
- **Version / run:** No MLflow run — nothing is trained. Reproduced by `scripts/d4_gdd.py` from config + public weather.
...
- **Negative finding, kept deliberately:** the computed véraison band crossings (around Sept 1–17, 2025) are almost certainly too late, because the cited literature bands accumulate from Jan 1 while this run's Winkler window starts Apr 1. The offset quantitatively demonstrates that transplanted literature thresholds cannot substitute for local calibration.
...
- Band-crossing dates are literature thresholds applied to local weather, not observed phenology — nobody has confirmed any vine at IHV did anything on those dates.
- The `sparkling_harvest_watch_UNCALIBRATED` band (1250–1500 GDD10) is a declared placeholder with no peer-reviewed source; it exists only as a slot to calibrate once real harvest dates arrive.
