"""Evaluate the CRPS skill ceiling and the FHS ensemble across probes (D2).

Runs the shared purged walk-forward harness on all five IHV soil probes and
writes the tidy results table to docs/reports/assets/d2_ceiling_results.csv.
Every model keeps persistence as the forecast center; the new columns report
the theoretical skill ceiling (a pure shape functional of the standardized
persistence errors) and each model's efficiency, skill divided by ceiling
(see `vine.d2_irrigation.ceiling`).

    uv run python scripts/d2_ceiling.py configs/d2_irrigation/ceiling.yaml
"""
...
from vine.d2_irrigation.ceiling import run_ceiling_experiment
...
def main() -> int:
    ...
    parser.add_argument(
        "config",
        nargs="?",
        default="configs/d2_irrigation/ceiling.yaml",
    ...
    out = REPO_ROOT / "docs" / "reports" / "assets" / "d2_ceiling_results.csv"
    ...
    if not args.no_mlflow:
        ...
        with mlflow.start_run(run_name="skill-ceiling") as run:
            mlflow.log_params({**cfg.model_dump(), "seed": seed, "sensors": list(frames)})
            mlflow.log_text(results.to_csv(index=False), "ceiling_results.csv")
            for row in results[results.model != "persistence-point"].itertuples():
                ...
                mlflow.log_metric(f"ceiling_{tag}", row.ceiling)
                mlflow.log_metric(f"efficiency_{tag}", row.efficiency)
                mlflow.log_metric(f"ceiling_oracle_{tag}", row.ceiling_oracle)
                mlflow.log_metric(f"efficiency_oracle_{tag}", row.efficiency_oracle)
